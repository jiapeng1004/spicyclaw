...
- 接收并校验请求（`@Valid` / `@Validated` + Jakarta Validation）
- 调用 Service，返回 DTO
- 声明 HTTP 语义（`@ResponseStatus`、路径、Produces）

**禁止**在 Controller 中：

- 业务判断、事务、数据库访问
- 手动拆解 DTO 字段再传给 Service（应整包传递 Request DTO）
...
| 层 | 包 | 职责 |
|----|-----|------|
| Controller | `*.controller` / `*Controller` | HTTP + 校验 + 委派 |
| Service | `*.service` | 业务逻辑、事务 |
| DTO | `*.dto` | 请求/响应契约 |
| Entity | `*.entity` | 持久化模型 |
